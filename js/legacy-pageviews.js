(() => {
  ...
  function applyLegacyPageViews() {
    const base = legacyPageViews[window.location.pathname];
    if (base === undefined) return;

    const el = document.getElementById('busuanzi_value_page_pv');
    if (!el || el.dataset.legacyPvApplied === '1') return;

    const text = el.textContent.trim().replace(/,/g, '');
    if (!/^\d+$/.test(text)) return;

    el.dataset.legacyPvApplied = '1';
    el.textContent = (base + Number(text)).toLocaleString('en-US');
  }

  const observer = new MutationObserver(applyLegacyPageViews);

  function start() {
    applyLegacyPageViews();
  ...
  document.addEventListener('pjax:success', applyLegacyPageViews);
})();